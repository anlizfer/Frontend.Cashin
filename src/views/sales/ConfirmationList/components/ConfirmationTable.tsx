...
import ConfirmationDeleteConfirmation from './ConfirmationDeleteConfirmation'
...
import ConfirmationChangeStatusConfirmation from './ConfirmationChangeStatusModal'
...
/*
public List<ConfirmationLineDtoResponse>? ConfirmationLines { get; set; }
*/
...
const ActionColumn = ({ row }: { row: Confirmation }) => {
    ...
    const onDelete = () => {
        ...
        dispatch(setSelectedConfirmation(row.id))
    }
...
const ConfirmationColumn = ({ row }: { row: Confirmation }) => {
...
const ConfirmationTable = () => {
    ...
    const [openModalStatusConfirmation, setOpenModalStatusConfirmation] = useState(false);
    const [statusConfirmationData, setStatusConfirmationDataApi] = useState<any[]>([]);

    useEffect(()=>{
        getStatusConfirmationData();
        
    },[]);

    const getStatusConfirmationData=async()=>{
        const dataStatusConfirmation:any = await apiGetStatusOrder();
        const dataStatus=dataStatusConfirmation.data;
        const dataStatusD:any=[];
        dataStatus.forEach((element:any) => {
            dataStatusD.push({label:element.id+" - "+element.name, value:element.id});
        });          
        setStatusConfirmationDataApi(dataStatusD);
        dispatch(setStatusConfirmationData(dataStatusD));       
    }
    ...
    const loading = useAppSelector(
        (state) => state.salesConfirmationList.data.loading
    )

    const data = useAppSelector(
        (state) => state.salesConfirmationList.data.ConfirmationList
    )


    const showModalStatus=(idOrder:any)=>{
        /*setOpenModalStatusConfirmation(true);*/
        console.log("Abra");
        
        dispatch(toggleStatusConfirmation(true))
        dispatch(setSelectedConfirmation(idOrder));
    }

    const onCloseDialogStatus=()=>{
        setOpenModalStatusConfirmation(false);        
    }
    ...
    const fetchData = () => { 
        let pageParams:GetConfirmationsRequest={ pageIndex, pageSize, sort, query, filterData,idCompany:companyDefault?.id,IdStatusOrder:[7,3]};
        dispatch(getConfirmations(pageParams))
    }

    const columns: ColumnDef<Confirmation>[] = useMemo(
        () => [
            {
                header: 'Ref',
            ...
            {
                header: '',
                accessorKey: 'statusConfirmationId',
                cell: (props) => {
                    const { statusOrder } = props.row.original
                    return <span
            ...
            },
            {
                ...
                cell: (props) => {
                    ...
                    return <ConfirmationColumn row={row} />
                },
            },
    ...
    return (
        <>
            <DataTable
                ...
                onSort={onSort}
            />
            <ConfirmationDeleteConfirmation />

            <ConfirmationChangeStatusConfirmation />
        </>
    )
}
